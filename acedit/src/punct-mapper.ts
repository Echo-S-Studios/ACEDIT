 ...
 */
const PUNCTUATION_MAPPINGS: Record<number, { name: string; ascii?: string }> = {
  // Spaces
 ...
 */
export function toASCII(codepoint: number): PunctuationResult {
 ...
 */
export function getPunctuationName(codepoint: number): string | null {
  if (!isPunctuation(codepoint)) {
    return null;
  }
 ...
 */
export function getAllPunctuationMappings(): Array<{
  codepoint: number;
  unicode: string;
  character: string;
  name: string;
 ...
 */
export function validatePunctuationUsage(input: string): PunctuationResult {
