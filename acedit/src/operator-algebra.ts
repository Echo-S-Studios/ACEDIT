...
import { isOperator, isModifier, getRegisterInfo } from './register-engine.js';
 ...
 */
export function composeOperators(op1: number, op2: number): CompositionResult {
 ...
 */
export function applyModifier(operator: number, modifier: number): CompositionResult {
 ...
 */
export function validateCompositionChain(codepoints: number[]): CompositionResult {
 ...
 */
export function isIdentityComposition(input: number, output: number): boolean {
  return input === output;
}
 ...
 */
export function getInverse(operator: number): CompositionResult {
  if (!isOperator(operator)) {
    return {
      valid: false,
      output: null,
