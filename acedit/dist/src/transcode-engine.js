 ...
 */
export function transcodeCodepoint(codepoint, targetRegister) {
 ...
 */
export function transcodeString(input, targetRegister) {
    if (input.length === 0) {
        return {
            valid: true,
            output: '',
 ...
 */
export function transcodeWithCoherenceValidation(input, targetRegister) {
 ...
 */
export function detectAndSuggestTranscode(input) {
    const registers = getStringRegisters(input);
 ...
 */
export function batchTranscode(inputs, targetRegister) {
    return inputs.map(input => transcodeString(input, targetRegister));
}
//# sourceMappingURL=transcode-engine.js.map